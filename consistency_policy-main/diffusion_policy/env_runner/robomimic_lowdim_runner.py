...
class RobomimicLowdimRunner(BaseLowdimRunner):
    ...
    def __init__(self, 
        ...
        dataset_path = os.path.join("./diffusion_policy/", dataset_path)
        # print(dataset_path)
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        # Create the environment from metadata
        env_name = self.env_meta["env_name"]
        env_kwargs = self.env_meta["env_kwargs"]
        env_kwargs["robots"] = ['IIWA', 'IIWA']

        print(env_kwargs["robots"])

        env_sim = robosuite.make(
            env_name=env_name,
            **env_kwargs,
        )
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                # print(len(np_action_dict['action'][0]))
                # print(len(np_action_dict))
                ...
                # print(len(env_action[0]))
                # print(env_action[0])
                # print(len(self.past_action))
                for i in range(len(env_action[0])):
                    # print(env_action[0][i])
                    obs_s, reward, done_s, info = env_sim.step(env_action[0][i])
                    sim.step()
                    viewer.render()
                    # time.sleep(0.01)
