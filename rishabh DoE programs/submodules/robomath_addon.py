...
def quaternions_2_eulers(quaternions: np.ndarray | list) -> np.ndarray:
    """
    Convert a quaternion to euler angles in radians.
    input: quaternions list of [(4,)] type np.ndarray
    output: euler angles list of [(3,)] type np.ndarray
    """
    eulers = [quaternion_2_euler(_qin) for _qin in quaternions]
    return np.array(eulers)



def XYZwxyz_2_Pose(XYZwxyz: list | np.ndarray) -> rm.Mat:
    '''
...
def Pose_2_XYZwxyz(pose: rm.Mat) -> np.ndarray:
